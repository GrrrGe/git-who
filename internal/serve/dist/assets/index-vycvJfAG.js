...
git-who serve --repo /path/to/repo</pre>
  ...
  <div class="wrap">
  ...
  </div>`}function ve(e,n){const r=Math.max(1,...n.buckets.map(a=>a.total)),o=n.buckets.reduce((a,d)=>d.value>((a==null?void 0:a.value)??-1)?d:a,n.buckets[0]),i=n.buckets.map(a=>{const d=a.value/r*100,f=(a.total-a.value)/r*100,s=`${a.period}: ${a.author.name||"(no commits)"} ${a.value}/${a.total}`;return`<div class="hist-row" role="img" aria-label="${p(s)}">
    ...
    <table class="data" aria-label="Contributions by author">
      ...
      <tbody id="author-rows">${P(o)}</tbody>
    </table></div>`,e.querySelectorAll("button[data-sort]").forEach(l=>{l.onclick=()=>{const u=l.dataset.sort;(t.tableSort||t.mode)===u?t.tableSortDir*=-1:(t.tableSort=u,t.tableSortDir=-1),r()}});const s=e.querySelector("#tf"),g=e.querySelector("#author-rows"),m=e.querySelector("#suggest");let b=-1;const M=()=>{const l=t.tableFilter.toLowerCase().trim();return l?n.authors.filter(u=>u.name.toLowerCase().includes(l)||u.email.toLowerCase().includes(l)).sort((u,c)=>E(u)-E(c)).slice(0,8):n.authors.slice(0,8)},E=l=>{const u=t.tableFilter.toLowerCase().trim();return l.name.toLowerCase().startsWith(u)?0:l.email.toLowerCase().startsWith(u)?1:2},A=l=>{const u=t.tableFilter.trim();if(!u)return p(l);const c=l.toLowerCase().indexOf(u.toLowerCase());return c<0?p(l):p(l.slice(0,c))+'<b class="hl">'+p(l.slice(c,c+u.length))+"</b>"+p(l.slice(c+u.length))},$=()=>{if(!m||!s)return;const l=M();l.length?(m.innerHTML=l.map((u,c)=>`
        ...
        </button>`).join(""),m.querySelectorAll(".suggest-item").forEach(u=>{u.addEventListener("mousedown",c=>{c.preventDefault(),D(l[Number(u.dataset.i)])})})):m.innerHTML='<div class="suggest-empty">No matching authors.</div>',m.hidden=!1,s.setAttribute("aria-expanded","true")},L=()=>{!m||!s||(m.hidden=!0,s.setAttribute("aria-expanded","false"),b=-1)},D=l=>{t.tableFilter=l.name,s&&(s.value=l.name),g&&(g.innerHTML=P(O(n))),L()};s==null||s.addEventListener("input",()=>{t.tableFilter=s.value,b=-1,g&&(g.innerHTML=P(O(n))),$()}),s==null||s.addEventListener("focus",()=>{b=-1,$()}),s==null||s.addEventListener("keydown",l=>{if(m!=null&&m.hidden)return;const u=M();l.key==="ArrowDown"?(l.preventDefault(),b=Math.min(b+1,u.length-1),$()):l.key==="ArrowUp"?(l.preventDefault(),b=Math.max(b-1,-1),$()):l.key==="Enter"&&b>=0&&u[b]?(l.preventDefault(),D(u[b])):l.key==="Escape"&&L()}),s==null||s.addEventListener("blur",()=>L())}function $e(e,n){if(!n)return e;let r=e;for(const o of n.split("/")){const i=(r.children||[]).find(a=>a.name===o&&a.is_dir);if(!i)return null;r=i}return r}function Se(){return t.mode==="lines"?"Lines":t.mode==="files"?"Files":t.mode==="last_modified"?"Last edit":t.mode==="first_modified"?"First edit":"Commits"}function Le(e){return t.mode==="lines"?`+${e.metrics.lines_added.toLocaleString()} / -${e.metrics.lines_removed.toLocaleString()}`:t.mode==="files"?e.metrics.files.toLocaleString():t.mode==="last_modified"?x(e.metrics.last_edit):t.mode==="first_modified"?x(e.metrics.first_edit):e.metrics.commits.toLocaleString()}function ke(e,n,r){if(!n.root){e.innerHTML="<p>No commits; tree is empty.</p>";return}const i=(($e(n.root,t.treePath)??n.root).children||[]).filter(s=>s.in_work_tree).sort((s,g)=>Number(g.is_dir)-Number(s.is_dir)||s.name.localeCompare(g.name)),a=t.treePath?t.treePath.split("/"):[],d=[`<button data-crumb="" class="crumb" aria-label="Repository root">${p(n.root.name==="."?t.repoName||"repo":n.root.name)}</button>`];a.forEach((s,g)=>{const m=a.slice(0,g+1).join("/");d.push(`<span aria-hidden="true">/</span> <button data-crumb="${p(m)}" class="crumb">${p(s)}</button>`)});const f=i.map(s=>{const g=s.is_dir?"📁":"📄",m=s.is_dir?`<button data-enter="${p(s.is_dir?t.treePath?t.treePath+"/"+s.name:s.name:"")}" class="linklike">${p(s.name)}</button>`:`<span>${p(s.name)}</span>`;return`<tr>
    ...
    </div>`,window.matchMedia("(prefers-reduced-motion: reduce)").matches)return()=>{};const o=B[n]??["Working"];let i=0;const a=e.querySelector("#index-status"),d=window.setInterval(()=>{i=(i+1)%o.length,a&&(a.textContent=`${o[i]}…`)},900);return()=>window.clearInterval(d)}async function qe(e){const n=De(e,t.view);try{t.view==="table"?we(e,await ae(),h):t.view==="tree"?ke(e,await ne(),h):ve(e,await re())}catch(r){e.innerHTML=`<p class="error" role="alert">Error: ${p(r.message)}</p>`}finally{n()}}function Y(e){t.guideFrom=e,t.screen="guide",h()}function Te(){t.screen=t.guideFrom==="app"&&t.repo?"app":"landing",h()}function h(){var n,r,o,i;const e=document.getElementById("app");if(t.screen==="guide"){e.innerHTML=fe(),me(e,Te),H();return}if(!t.repo){t.screen="landing",e.innerHTML=be(),ge(e,U,()=>Y("landing")),H();return}t.screen="app",e.innerHTML=ye(),(n=e.querySelector("#brand-home"))==null||n.addEventListener("click",()=>{t.repo="",t.repoName="",t.analyzeError="",h()}),(r=e.querySelector("#brand-q"))==null||r.addEventListener("click",()=>Y("app")),(o=e.querySelector("#new-repo"))==null||o.addEventListener("click",()=>{t.repo="",t.repoName="",t.analyzeError="",h()}),(i=e.querySelector("#theme"))==null||i.addEventListener("click",()=>{const a=document.documentElement,d=a.getAttribute("data-theme")==="dark"?"light":"dark";a.setAttribute("data-theme",d),localStorage.setItem("git-who-theme",d)}),e.querySelectorAll("button[data-view]").forEach(a=>{a.onclick=()=>{t.view=a.dataset.view,h()}}),e.querySelectorAll("button[data-mode]").forEach(a=>{a.onclick=()=>{t.mode=a.dataset.mode,t.tableSort="",h()}}),pe(e,h),document.onpointerdown=a=>{t.filtersOpen&&!a.target.closest(".popover-wrap")&&(t.filtersOpen=!1,h())},document.onkeydown=a=>{a.key==="Escape"&&t.filtersOpen&&(t.filtersOpen=!1,h())},qe(e.querySelector("#view")),H()}async function U(e){t.analyzing=!0,t.analyzeError="",t.landingInput=e,h();try{const n=await se(e);t.repo=n.repo,t.repoName=n.name,t.treePath="",t.collapsed.clear(),t.analyzing=!1;const r=new URL(location.href);r.searchParams.set("repo",e),history.replaceState(null,"",r),h()}catch(n){t.analyzing=!1,t.analyzeError=n.message,h()}}function xe(){Q(),Z();const e=localStorage.getItem("git-who-theme");e&&document.documentElement.setAttribute("data-theme",e);const r=new URLSearchParams(location.search).get("repo");if(r){t.landingInput=r,U(r);return}h()}xe();
